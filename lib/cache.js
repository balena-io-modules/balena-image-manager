 ...
 */
export async function getImagePath(deviceType, version) {
 ...
 */
export async function isImageFresh(deviceType, version) {
...
export async function getImageWritableStream(deviceType, version) {
	...
	// @ts-ignore adding an extra prop
	stream.persistCache = () => fs.promise.rename(inProgressPath, imagePath);

	// @ts-ignore adding an extra prop
	stream.removeCache = () => fs.promise.unlink(inProgressPath);
