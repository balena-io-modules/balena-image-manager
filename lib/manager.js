...
const doDownload = (deviceType, version) =>
	balena.models.os.download(deviceType, version).then(function (imageStream) {
		...
		return cache
			.getImageWritableStream(deviceType, version)
			.then(function (cacheStream) {
				...
				imageStream.on('error', (err) =>
					cacheStream.removeCache().then(() => pass2.emit('error', err)),
				);
 ...
 * Defaults to `'latest'`.
 * @returns {Promise<ReadableStream>} image readable stream
 *
 * @example
 * manager.get('raspberry-pi', 'default').then (stream) ->
 * 	stream.pipe(fs.createWriteStream('foo/bar.img'))
 */
export function get(deviceType, versionOrRange) {
	if (versionOrRange == null) {
		versionOrRange = 'latest';
	}
	return utils.resolveVersion(deviceType, versionOrRange).then((version) =>
		// @ts-expect-error passthrough stream as readable stream
		cache.isImageFresh(deviceType, version).then(function (isFresh) {
			if (isFresh) {
